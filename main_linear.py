...
def parse_option():
	...
	# model dataset
	parser.add_argument('--model', type=str, default='inceptionresnetv1')
	parser.add_argument('--dataset', type=str, default='affwild2',
	...
	parser.add_argument('--size', type=int, default=160)  # image size
...
def set_loader(opt):
	...
	if opt.dataset == 'cifar10':
		...
		train_dataset = get_affwild2_dataset('train', train_transform)
		val_dataset = get_affwild2_dataset('val', val_transform)
	else:
...
def set_model(opt):
	model = SupCEResNet(name=opt.model, num_classes=opt.n_cls, use_webface_pretrain=True)
	model.encoder.requires_grad_(False)  # frozen encoder, only train classifier
...
def main():
	best_acc = 0
	opt = parse_option()
	trial_name = f"m{opt.model}_lr{opt.learning_rate}_decay{opt.weight_decay}_bs{opt.batch_size}_ep{opt.epochs}_opt-AdamW_sch-linwp{opt.warm_epochs}_trial{opt.trial}"
	writer = SummaryWriter(osp.join('runs/linear',trial_name))
	log.info(f"***********\n TRIAL: {trial_name}\n STARTS!***********")
	print(f"***********\n TRIAL: {trial_name}\n STARTS!***********")
